...
`Relational Databases` are a kind of database structured as rows within a set of columns that define a particular context whereas SQL `(Structured Query Language)` is not a database, but a language for accessing and interacting with data in Relational Databases.

1. Why do tables need a `primary key`?
Tables have primary keys in order to ensure that data inn each column is unique
